...
function App( {isOpen ,setIsSideDrawerOpen}) {
    ...
    return (
      <div className="App2">
        ...
        <button className="CloseButton" onClick={() => setIsSideDrawerOpen(false)}>
          &#x2715;
        </button>
          {problems.map((problem) => (
            <button key={problem.name} onClick={() =>{
                handleClick(problem) 
                setIsShow(true)
            } }>
              {problem.name}
        ...
        { isShow && <button className="CloseButton" onClick={() => setIsShow(false)}>
          &#x2715;
        </button> }
